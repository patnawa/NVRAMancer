...
//
//CR คือ configuration register ซึ่งบางยี่ห้อเก็บทิศทางการล็อกไว้ในนั้น
//Macronix อ่านด้วย 15h บิต 3 คือ TB ส่วน Spansion อ่านด้วย 35h บิต 5 คือ TBPROT
...
begin
  ...
  //WPS ยกให้บิตล็อกรายบล็อกเป็นตัวตัดสิน ซึ่งทำให้บิต BP ที่เพิ่งล้างไม่มีผล
  //
  //QE อยู่ที่บิต 1 และต้องอยู่ต่อ ทุกบิตที่เหลือก็เก็บไว้ตามเดิม
  NewSR2 := SR2 and (not byte(%01000100));
end;
...
begin
  ...
  if (P.BPBits <> 4) and (P.BP = 7) and (not P.SEC) then
  begin
