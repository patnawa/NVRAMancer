...
stage-9 production suite runs live on Linux (`tools/build.sh`). Remaining:
step 3 (CH347 libusb backend, CLI-only Linux build — needs CH347 hardware on
a Linux box to validate) and step 4 (UsbAsp/AVRISP via libusb, GUI last).
